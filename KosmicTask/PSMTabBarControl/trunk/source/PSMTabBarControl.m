...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    if (cell) {
		...
		if (distance >= 10 && !_didDrag && ![[PSMTabDragAssistant sharedDragAssistant] isDragging] &&
				...
				[_cells count] > 1	// dragging just one cell is a killer
			) {
    ...
    if ([[[sender draggingPasteboard] types] indexOfObject:@"PSMTabBarControlItemPBType"] != NSNotFound) {
		...
		if ((GetCurrentEventKeyModifiers() == 0) && bit_test(keymap.bits, virtualKeycodeForSpace)) {
			...
			if (!_springTimer) {
				...
				NSNumber *delayNumber = [NSMakeCollectable(CFPreferencesCopyAppValue((CFStringRef)@"SpringingDelayMilliseconds", (CFStringRef)@"com.apple.finder")) autorelease];
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)aTabView
{
	...
	// with GC and leopard this produces a mutation exception
	...
	// exception array mutated during enumeration
	// **** JM modified 160108 - START
/*	int i;
	PSMTabBarCell *cell;
	for (i=[_cells count]-1; i >=0; i--) {
