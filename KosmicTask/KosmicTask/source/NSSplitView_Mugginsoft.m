...
- (void)resizeSubviewsWithOldSize:(NSSize)oldSize withBehaviour:(MGSSplitviewBehaviour)behaviour minSizes:(NSArray *)minSizes
{
	...
	if (minSizes) {
		...
		if ([minSizes count] >= 1) minSize0 = [[minSizes objectAtIndex:0] floatValue];
		if ([minSizes count] >= 2) minSize1 = [[minSizes objectAtIndex:1] floatValue];
...
/*
 ...
 - restoreSubviewFramesWithDefaultsName:
 
 */
- (void)restoreSubviewFramesWithDefaultsName:(NSString *)name
{
	NSArray *frameStrings = [[NSUserDefaults standardUserDefaults] arrayForKey:name];
	if (!frameStrings) return;
	
	NSInteger indx = 0;
	NSInteger maxIndx = [[self subviews] count] - 1;
	
	for (NSString *rectString in frameStrings) {
		NSRect rect = NSRectFromString(rectString);
		[[[self subviews] objectAtIndex:indx] setFrame:rect];
		if (++indx > maxIndx) {
			break;
		}
	}
...
- saveSubviewFramesWithDefaultsName:
...
- (void)saveSubviewFramesWithDefaultsName:(NSString *)name
{
	...
	NSMutableArray *frameStrings = [NSMutableArray arrayWithCapacity:2];
	for (NSView *subView in [self subviews]) {
		NSString *frameString = NSStringFromRect([subView frame]);
		[frameStrings addObject:frameString];
	}
	[[NSUserDefaults standardUserDefaults] setObject:frameStrings forKey:name];
}
@end
