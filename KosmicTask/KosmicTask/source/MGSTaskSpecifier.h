...
@interface MGSTaskSpecifier : NSObject <NSCopying> {
	MGSNetClient *__weak _netClient;			// net client to initiate task on
	...
	/* when a client disconnects and reconnects when a task is visible the netClient
	 ...
	 a proxy netClient */
	MGSNetClientProxy *__weak _representedNetClient; // proxy client used for binding
	
	int _scriptIndex;	// index of the task script within the net client script handler
	MGSScript *__weak _script;	// a deep copy of the task script indicated by _scriptIndex
	MGSTaskDisplayType _displayType;	// display in selected or new tab
	NSInteger _identifier;	// numeric identifier
	
	MGSClientNetRequest *__weak _netRequest;				// currently active request for this task
...
@property (weak) MGSNetClientProxy *representedNetClient;
@property NSTimeInterval elapsedTime;
@property NSTimeInterval remainingTime;
@property (weak) MGSClientNetRequest *netRequest;
