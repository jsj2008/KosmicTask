...
- (id) initWithDelegate:(id)delegate userData:(long)userData
{
	...
	disconnectCalled = NO; // JM 23-02-08 GC compatibility
	return self;
}
// JM 23-11-07 GC compatability
...
// the old dealloc cannot become finalize
/*
Design Patterns

Object finalization occurs at most once during the lifetime of an objectÑwhen it is collected. When more than one object is finalized, 
 the order in which they are sent a finalize message is indeterminate, even if there are references between them. If you send messages
 between objects when they are being finalized, you must take extreme care to avoid anomalous behavior. To ease correctness concerns 
 alone, it is best not to attempt any work in a finalizer. Moreover, however, time spent in object finalization incurs application
 overhead. Your design goal should therefore be to not have a finalizer at all. If you must use a finalizer, you should keep it as
 short as possible, and reference as few other objects as possible in its implementation.

Efficiency
Memory recovery time is typically not the best time to reclaim resources or do clean-up work 
 (such as releasing instance variables and closing resources). Your finalize code is part of the garbage collectorÕs critical path, 
 and so should be kept to a minimum if not eliminated entirely. You should implement invalidation code that is distinct from your 
 deallocation or finalization code and invoke it when appropriate.
...
You should typically use NSMakeCollectable() on Core Foundation objects rather than relying on CFRelease() in 
 finalizeÑthis way collectable Core Foundation objects are actually collected sooner. 
 (Collectable objects are collected with the source object whereas released objects are simply 
 marked as being eligible for collectionÑthese must wait for the next collection cycle to be collected.)

Messaging Other Objects
No objects are deallocated until all finalizers are complete (otherwise, no finalizer could use any other object anywhere, 
 including objects like NSString that donÕt have a finalizer) so you can access already-finalized objectsÑbut only in other
 finalizers. Within a finalize method, therefore, you should reference as few other objects as possible.
 You can't necessarily know what other objects might have a reference to your instance, or whether they might message your 
 instance from their finalizer, you must therefore code defensively to try to keep your instance as fully functional as is possible
 to support messages it might receive after finalization. Similarly, since you don't know in what order objects will be finalized, 
 it may be that objects you message during a finalize method have themselves already been cleared.

For example, some objects use collection objects (arrays, dictionaries, or sets) to hold other related objects. Sometimes during 
 finalization the collection is accessed and messages sent to each and every contained object. If the collection itself had been 
 and had discharged its objects, the algorithm would fail on that account alone. Similarly, if any of the objects in the collection 
 can no longer respond correctly to the requested message after it is finalized, the algorithm again will fail.

Avoiding Resurrection
Some Cocoa objects make assumptions about how many references are kept about themselves and where, for example by implementing 
 the release method to trap the transition to a known value (typically of 0) and then distributing cleanup work among their holders. 
 In a garbage-collected environment, this pattern can lead to ÒresurrectionÓ of an objectÑthat is, it becomes valid again after 
 having been finalized.

Resurrection occurs when a finalize method stores self in a non-garbage object. The resurrected object becomes a zombie. 
 It logs all messages that are sent to it, but it is otherwise useless. It is eventually deallocated when it becomes garbage again 
 (when its container is collected). You should consider resurrection to be a programming error.
*/
// see the following:
//file://localhost/Developer/Documentation/DocSets/com.apple.ADC_Reference_Library.CoreReference.docset/Contents/Resources/Documents/documentation/Cocoa/Conceptual/GarbageCollection/index.html
- (void) disconnect
{
	if (disconnectCalled == YES) {
		NSLog (@"AsyncSocket repeat call to disconnect");
		return;
	}
	...
	theReadQueue = nil;
	theWriteQueue = nil;
	//[NSObject cancelPreviousPerformRequestsWithTarget: theDelegate selector: @selector(onSocketDidDisconnect:) object:self]; // JM 25-02-08
	//[NSObject cancelPreviousPerformRequestsWithTarget: self]; // JM 25-02-08
}

// JM 23-11-07
// A resurrection error was occurring if called dispose during finalize
// This error occurs when an object that has received a finalize message is referenced
...
Failed:;
	//[self close]; // JM 25-02-08 GC compatibility
	[self disconnect]; // JM 25-02-08 GC compatibility
	return NO;
}
...
{
	// [self close]; // JM 25-02-08 GC compatibility
	[self closeAndDispose]; // JM 25-02-08 GC compatibility
}*/
...
- (void) doStreamOpen
{
	...
	if ([self areStreamsConnected] && !(theFlags & kDidCallConnectDeleg))
	{
		...
		// JM occasion when this was nil 24-04-08
		if (nil == peer) {
			err = [self getSocketError];
			NSLog (@"AsyncSocket %p couldn't get remote socket address from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
		}
		// end JM
...
- (void) close
{
	...
	if (theFlags & kDidPassConnectMethod)
	{
		...
		if ([theDelegate respondsToSelector: @selector(onSocketDidDisconnect:)])						
			[theDelegate performSelector:@selector(onSocketDidDisconnect:) withObject:self];
	}
...
#pragma warning - failure here occasionally - delegate isa is 0xfffff
