...
static unsigned long int numberOfTimesModified( OSAID aScriptID, ComponentInstance aComponentInstance ) __unused;
static BOOL isTypeCompiledScript( OSAID aScriptID, ComponentInstance aComponentInstance );
static BOOL isTypeScriptValue( OSAID aScriptID, ComponentInstance aComponentInstance ) __unused;
...
- (NSAttributedString *)attributedSource
{
	...
	NDLogOSAError(OSACopySourceString([self instanceRecord],[self scriptID], kOSAModeNull, &result)); 
...
- (NSDictionary *)error
{
	return [[self componentInstance] error];
}
...
- (void)finalize
{
	// call dispose when done with object rather than depending on finalize
	/*if( scriptID != kOSANullScript )
		NDLogOSAError( OSADispose( [self instanceRecord], scriptID ));*/
	[super finalize];
}
...
- (NSData *)data
{
	...
	// store into a descriptor
	BOOL descValid = NDLogOSAError(OSAStore( [self instanceRecord], [self scriptID], typeOSAGenericStorage, kOSAModeDontStoreParent, &theDesc ));
	
	// get a data representation
	if (descValid) {
		data = [[NSAppleEventDescriptor descriptorWithAEDesc:&theDesc] data];
	} 
...
- (NSAppleEventDescriptor *)appleEventDescriptorValue
{
	...
	return NDLogOSAError( OSACoerceToDesc( [self instanceRecord], [self scriptID], typeWildCard, kOSAModeNull, &theDesc))
		? [NSAppleEventDescriptor descriptorWithAEDesc:&theDesc] 
		: nil;
}
...
- (NSString *)stringValue
{
	...
	return theSuccess ? [[NSAppleEventDescriptor descriptorWithAEDesc:&theDesc] stringValue] : nil;
}
...
- (NSString *)description
{
	...
	return NDLogOSAError( OSADisplay( [self instanceRecord], [self scriptID], kTXNUnicodeTextData, kOSAModeDisplayForHumans, &theDesc ))
		? [NSString stringWithFormat:@"%s: %@", theTypeStr, [[NSAppleEventDescriptor descriptorWithAEDesc:&theDesc] stringValue]]
		: nil;
}
...
{
	return aComponentInstance == [self componentInstance] ? [self copyWithZone:nil]: [[NDScriptData scriptDataWithScriptID:[self scriptID] componentInstance:aComponentInstance] retain];
}
...
- (id)initWithSource:(NSString *)aSource modeFlags:(SInt32)aModeFlags componentInstance:(NDComponentInstance *)aComponentInstance
{
	...
	return [self initWithScriptID:[[self class] compileString:aSource modeFlags:aModeFlags scriptID:kOSANullScript componentInstance:aComponentInstance] componentInstance:aComponentInstance];
}
...
- (NSString *)source
{
	...
	return canGetSource( [self scriptID], [self instanceRecord] ) && NDLogOSAError( OSAGetSource( [self instanceRecord], [self scriptID], kTXNUnicodeTextData, &theDesc))
			? [[NSAppleEventDescriptor descriptorWithAEDesc:&theDesc] stringValue]
			: nil;
}
...
/*
 ...
 result error
 */
- (OSAError)resultError
{
	return resultOSAError;
}
 ...
 */
- (NSString *)resultErrorString
{
	return stringForOSAError(resultOSAError);
}
...
- (BOOL)executeEvent:(NSAppleEventDescriptor *)anEvent
{
	...
	// 
	// MGS 14-10-08
	// OSAExecuteEvent may return errAEEventNotHandled or errOSAScriptError
	// If the return value is errOSAScriptError then OSAScriptError() may be called to determine the error.
	// If the return value is errAEEventNotHandled then no error is reported by OSAScriptError.
	// resultOSAError added to track OSAFunction return value
	//
	theResult = NDLogOSAError( resultOSAError = OSAExecuteEvent([self instanceRecord], [anEvent aeDesc], [self scriptID], [self executionModeFlags], &theResultScriptID) );
...
#pragma mark -
#pragma mark Execution mode flags
/*
...
#pragma mark -
#pragma mark Execution handling
/*
	-  executeOpen:
 */
...
/*
 ...
 MGS 13-10-08
 */
- (BOOL)executeAppOpen:(NSArray *)aParameters
{
	NSAppleEventDescriptor	* theEvent = nil;
	theEvent = [NSAppleEventDescriptor openAppEventDescriptorWithTargetDescriptor:[self appleEventTarget] array:aParameters];
...
{	
	...
	{
		NDLogOSAError( OSALoad([aComp instanceRecord], [theDataDesc aeDesc], aModeFlags, &aScriptID) );
	}
