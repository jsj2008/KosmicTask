	...
	else if( strcmp( theObjCType, @encode( NSRange ) ) == 0 )
	{
		...
		if( AECreateDesc( keyAERangeStart, (void*)&theRange.location, sizeof(unsigned int), &theValues[0] ) == noErr )
		{
			if( AECreateDesc( keyAERangeStop, (void*)&theRange.length, sizeof(unsigned int), &theValues[1] ) == noErr )
			{
				...
				{
					theDescriptor = [NSAppleEventDescriptor descriptorWithAEDesc:&theDesc];
				}
				else
...
/*
 ...
 for additional examples or coercion see
 
 http://homepage.mac.com/kenferry/Software/KFASHandlerAdditions/releases/KFASHandlerAdditions-2.3.zip
...
+ (NSAppleEventDescriptor *)descriptorWithObject:(id)anObject
{
	...
	{
		theDescriptor = [self descriptorWithDate:anObject];
	}
...
/*
 ...
 see http://homepage.mac.com/kenferry/software.html
 KFAppleScript Handler Additions
 
 */
+ (NSAppleEventDescriptor *)descriptorWithDate:(NSDate *)aDate
{
	LongDateTime ldt;
	
	UCConvertCFAbsoluteTimeToLongDateTime(CFDateGetAbsoluteTime((CFDateRef)aDate), &ldt);
	return [[self alloc] initWithDescriptorType: 'ldt '
								  bytes: &ldt
								 length: sizeof(ldt)];
}
...
/*
 - openAppEventDescriptorWithTargetDescriptor:array:
 MGS 13-10-08
 */
+ (NSAppleEventDescriptor *)openAppEventDescriptorWithTargetDescriptor:(NSAppleEventDescriptor *)aTargetDescriptor array:(NSArray *)anArray 
{
	NSAppleEventDescriptor	* theEvent = nil,
	* theEventList = nil;
	...
	if( aTargetDescriptor != nil)
	{
		theEventList = [NSAppleEventDescriptor descriptorWithArray:anArray];
...
- (NSAppleEventDescriptor *)sendWithSendMode:(AESendMode)aSendMode sendPriority:(AESendPriority)aSendPriority timeOutInTicks:(SInt32)aTimeOutInTicks idleProc:(AEIdleUPP)anIdleProc filterProc:(AEFilterUPP)aFilterProc
{
	...
	return [NSAppleEventDescriptor descriptorWithAEDesc:&theResult];
}
