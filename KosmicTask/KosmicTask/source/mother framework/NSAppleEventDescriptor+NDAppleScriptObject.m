...
-(NSDictionary *)dictionaryValueFromRecordDescriptor
{
	...
	for( theIndex = 1; theIndex <= theNumOfItems; theIndex++ )
	{
		...
		// JM 09-05-08 exception being thrown when retrieving record for
		// applescript "system info"
		// object value is nil for home directory alias record item
		id objectValue = [theObject objectValue];
		if (objectValue) {
			//NSLog(@"AERecord class is %@", [objectValue className]);
			//[theDictionary setObject:objectValue forKey:[NSNumber numberWithUnsignedInt:theKeyword]];
			
			// try with key as string.
			// cannot convert to plist otherwise
			[theDictionary setObject:objectValue forKey:[NSString stringWithFormat: @"%@", [NSNumber numberWithUnsignedInt:theKeyword]]];
		}
	}
