...
#define SCOPE_BAR_H_INSET				8																	// inset on left and right
#define SCOPE_BAR_HEIGHT				25																	// used in -sizeToFit
#define SCOPE_BAR_START_COLOR_GRAY		[NSColor colorWithCalibratedWhite:0.75f alpha:1.0f]						// bottom color of gray gradient
#define SCOPE_BAR_END_COLOR_GRAY		[NSColor colorWithCalibratedWhite:0.90f alpha:1.0f]						// top color of gray gradient
#define SCOPE_BAR_START_COLOR_BLUE		[NSColor colorWithCalibratedRed:0.71f green:0.75f blue:0.81f alpha:1.0f]	// bottom color of blue gradient
#define SCOPE_BAR_END_COLOR_BLUE		[NSColor colorWithCalibratedRed:0.80f green:0.82f blue:0.87f alpha:1.0f]	// top color of blue gradient
#define SCOPE_BAR_BORDER_COLOR			[NSColor colorWithCalibratedWhite:0.69f alpha:1.0f]						// bottom line's color
#define SCOPE_BAR_BORDER_WIDTH			1																		// bottom line's width

#define SCOPE_BAR_SEPARATOR_COLOR		[NSColor colorWithCalibratedWhite:0.52f alpha:1.0f]	// color of vertical-line separators between groups
#define SCOPE_BAR_SEPARATOR_WIDTH		1													// width of vertical-line separators between groups
#define SCOPE_BAR_SEPARATOR_HEIGHT		16												// separators are vertically centered in the bar

#define SCOPE_BAR_LABEL_COLOR			[NSColor colorWithCalibratedWhite:0.45f alpha:1.0f]	// color of groups' labels
#define SCOPE_BAR_FONTSIZE				12												// font-size of labels and buttons
#define SCOPE_BAR_ITEM_SPACING			6													// spacing between buttons/separators/labels
#define SCOPE_BAR_BUTTON_IMAGE_SIZE		16												// size of buttons' images (width and height)
...
#define MENU_PADDING					25					// how much wider a popup-button is than a regular button with the same title.
#define MENU_MIN_WIDTH					60					// minimum width a popup-button can be narrowed to.
		...
		if ([delegate respondsToSelector:@selector(accessoryViewForScopeBar:)]) {
			...
			if (_accessoryView) {
				...
				*/
				...
				frame.origin.x = roundf(NSMaxX([self bounds]) - (frame.size.width + SCOPE_BAR_H_INSET));
				frame.origin.y = roundf(((SCOPE_BAR_HEIGHT - frame.size.height) / 2.0f));
...
- (void)sizeToFit
{
	...
	if ((NSInteger)frame.size.height != (NSInteger)SCOPE_BAR_HEIGHT) {
...
- (void)adjustSubviews
{
	...
	*/
	...
	if (((int)viewWidth == (int)_lastWidth) && ((int)_lastWidth != NSNotFound)) {
		return;
	}
	
	// Determine whether we got narrower or wider.
	BOOL narrower = (((int)_lastWidth == NSNotFound) || (viewWidth < _lastWidth));
	
	// Find width available for showing groups.
	float availableWidth = viewWidth - (SCOPE_BAR_H_INSET * 2.0f);
	if (_accessoryView) {
	...
	if ([_groups count] > 0) {
		...
		if (menuMode) {
			...
			if ([[group objectForKey:GROUP_BUTTONS] count] > 0) {
	...
	if (proceed) {
		...
		for (NSDictionary *groupInfo in groupsEnumerator) {
			...
			if (_firstCollapsedGroup == NSNotFound) {
				_firstCollapsedGroup = ((int)(narrower) ? (int)[_groups count] : -1);
				oldFirstCollapsedGroup = _firstCollapsedGroup;
			}
			_firstCollapsedGroup += ((int)(narrower) ? -1 : 1);
		...
		if (adjusting || shouldAdjustPopups) {
				...
				for (int i = (int)(changedRange.location); i < (int)NSMaxRange(changedRange); i++) {
					...
					if (narrower) {
						...
						for (int j = 0; j < (int)[menuItems count]; j++) {
							...
							[menuItems replaceObjectAtIndex:j withObject:button];
			...
			float startIndex = MIN((int)(changedRange.location), _firstCollapsedGroup);
			float xCoord = 0;
			...
			if (!narrower) {
				if (_firstCollapsedGroup >= (int)[_groups count]) {
					_firstCollapsedGroup = NSNotFound;
				}
			}
		}
...
- (NSButton *)getButtonForItem:(NSString *)identifier inGroup:(int)groupNumber
{
	...
	if (group && (int)[group count] > groupNumber) {
				  ...
				  withTitle:(NSString *)title image:(NSImage *)image
{
	...
	[[button cell] setFocusRingType:NSFocusRingTypeNone];
	...
	[button sizeToFit];
	ctrlRect = [button frame];
	ctrlRect.origin.y = floorf(([self frame].size.height - ctrlRect.size.height) / 2.0f);
	[button setFrame:ctrlRect];
...
- (NSPopUpButton *)popupButtonForGroup:(NSDictionary *)group
{
	float popWidth = floorf([[group objectForKey:GROUP_WIDEST_BUTTON_WIDTH] floatValue] + MENU_PADDING);
...
- (void)updateMenuTitleForGroupAtIndex:(int)groupNumber
{
	...
	if (groupNumber < 0 || groupNumber >= (int)[_groups count]) {
		return;
	}
...
- (void)drawRect:(NSRect)rect
{
	...
	if ([_separatorPositions count] > 0) {
		...
		sepRect.origin.y = (([self bounds].size.height - sepRect.size.height) / 2.0f);
...
- (void)setSelected:(BOOL)selected forItem:(NSString *)identifier inGroup:(int)groupNumber
{
	...
	if (identifier && groupNumber >= 0 && groupNumber < (int)[_groups count]) {
		NSDictionary *group = [_groups objectAtIndex:groupNumber];
...
- (void)updateSelectedState:(BOOL)selected forItem:(NSString *)identifier inGroup:(int)groupNumber informDelegate:(BOOL)inform
{
	...
	if (_selectedItems && (int)[_selectedItems count] > groupNumber) {
